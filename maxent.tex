...
\begin{empheq}[box=\tcbhighmath]{equation}
  \begin{split}
    ...
    &	   - \frac{d}{dx} \left( C \eta^\prime \right)  + 
	   \left( A - B^\prime \right) \eta = 0
   \end{split}
\end{empheq}
\end{tcolorbox}
...
\begin{equation*}
    \begin{align}
        Q(p,\eta) & = \lim_{\epsilon \to 0}  \frac{d^2}{d\epsilon^2} 
                 ...
                 \begin{rcases}
                     \begin{split}
                     & \int f(x) \left\{ p(x)+\epsilon \eta(x) \right\} dx
                           + T \int  \left\{ p(x) + \epsilon \eta(x) \right\} 
                              ln \left\{ p(x)  + \epsilon \eta(x) \right\}dx \\ 
                         &\qqad\qquad      + \int \left\{ p(x)+\epsilon \eta(x) \right\}
                         dx \right\} 
                     \end{split}
                 \end{rcases}
                 \end{cases}
                  ...
                  \left\{
                    \int f(x) \eta(x) dx
                      + T \int   \eta(x) \left\{
                                      ln \left\{ p(x)  + \epsilon \eta(x) \right\} + 1
                                      \right\} dx  \right}
    \end{align}
\end{equation*}
